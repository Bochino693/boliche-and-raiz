...
Write-Host "DRAGON BOWLING - APK PRO ULTRA 4K ANDROID 10" -ForegroundColor Cyan
...
if (-not (Test-Path (Join-Path $PastaTemplates "android_release.apk"))) {
    ...
    $Temporaria = Join-Path $env:TEMP "dragon-bowling-godot-templates"
...
if ($Instalar) {
    ...
    & $Adb install -r $Apk
    & $Adb shell monkey -p com.lazersport.dragonbowling -c android.intent.category.LAUNCHER 1
}
